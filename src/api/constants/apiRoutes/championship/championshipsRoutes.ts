export const ChampionshipsRoutes = {
    CHAMPIONSHIPS: '/',
    CHAMPIONSHIPS_ACTIVE: '/active',
    CHAMPIONSHIPS_ID: '/:championshipId',
    CHAMPIONSHIPS_BASIC_UPDATE: '/:championshipId',
    CHAMPIONSHIPS_DELETE: '/:championshipId',
    CHAMPIONSHIPS_ID_PHASES: '/:id/phases',
    CHAMPIONSHIPS_ID_PHASES_ID: '/:id/phases/:phaseId',
    CHAMPIONSHIPS_ID_PHASES_ID_MATCHES: '/:id/phases/:phaseId/matches',
    CHAMPIONSHIPS_ID_PHASES_ID_MATCHES_ID: '/:id/phases/:phaseId/matches/:matchId',
    CHAMPIONSHIPS_ID_TEAMS: '/:id/teams',
    CHAMPIONSHIPS_ID_CONFIGURATION: '/championship/:championshipId/configuration/:configurationId',
    CHAMPIONSHIPS_UPLOAD_LOGO: '/:championshipId/logo',
    CHAMPIONSHIPS_UPLOAD_BANNER: '/:championshipId/banner'
}
